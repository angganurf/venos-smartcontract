...
contract ShadowPool is Ownable, Pausable {
  ...
  function unlock(
    address _user
  ) external onlyOperatorOrShdwOwner(_user) whenNotPaused {
    UserInfo storage user = userInfo[_user];
    require(
   ...
   * @dev Only callable by the contract admin.
   */
  function setPerformanceFeeContract(
   ...
   * @dev Only callable by the contract admin.
   */
  function setWithdrawFeeContract(
  ...
  function getPricePerFullShare() external view returns (uint256) {
    return
      totalShares == 0
        ? 1e18
        : (((balanceOf() + calculateTotalpendingShdwRewards()) * (1e18)) /
          totalShares);
  }
